...
final class SubcommandRegistrationTests: XCTestCase {
    ...
    func testTotalSubcommandCount() throws {
        let config = LungfishCLI.configuration
        // 40 subcommands: version, convert, analyze, translate, search, universal-search, extract, fastq,
        // workflow, run-headless, fetch, bundle, project, provision-tools, conda, blast, esviritu,
        // taxtriage, align, msa, tree, assemble, orient, map, import, import-fastq, ops, provenance,
        // bam, variants, gatk, nao-mgs, freyja, nvd, cz-id, metadata, build-db, markdup, primers, debug.
        XCTAssertEqual(config.subcommands.count, 40)
    }
