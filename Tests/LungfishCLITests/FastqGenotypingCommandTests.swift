...
final class FastqGenotypingCommandTests: XCTestCase {
    func testFastqCommandRegistersPlatformNeutralGenotype() {
        ...
        XCTAssertFalse(names.contains("update-current-workbook"))
    ...
    func testRetiredWorkbookUpdateCommandIsRejected() {
        XCTAssertThrowsError(try FastqCommand.parseAsRoot(["update-current-workbook", "/tmp/historical.lungfishgenotype"]))
    }
