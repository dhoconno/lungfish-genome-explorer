...
/// Mirrors `FastqGenotypingBundleReferenceTests` for the two sibling genotyping
/// subcommands, asserting that the `.lungfishmhcref` consume path is shared.
final class FastqSiblingGenotypingBundleReferenceTests: XCTestCase {
    // MARK: - Help text

    func testONTBarcodeReferenceHelpMentionsLungfishMHCRef() {
        XCTAssertTrue(FastqONTBarcodeGenotypingSubcommand.referenceHelp.contains(".lungfishmhcref"))
    }

    func testONTReferenceHelpMentionsLungfishMHCRef() {
        XCTAssertTrue(FastqONTGenotypingSubcommand.referenceHelp.contains(".lungfishmhcref"))
    }

    // MARK: - ont-barcode-genotype bundle resolution

    func testONTBarcodeExplicitHaplotypeDefinitionMustExistInBundle() throws {
        ...
        XCTAssertThrowsError(
            try FastqONTBarcodeGenotypingSubcommand.resolveBundleHaplotypeDefinition(
    ...
    func testONTBarcodeExplicitHaplotypeDefinitionResolvesNonDefaultInBundle() throws {
        let bundleURL = try makeBundle(
            definitions: [Self.definition(id: "D1"), Self.definition(id: "D2")],
            defaultID: "D1"
        )

        let resolved = try FastqONTBarcodeGenotypingSubcommand.resolveBundleHaplotypeDefinition(
            referenceURL: bundleURL,
            explicitID: "D2"
        )
    ...
    func testONTBarcodeNilExplicitHaplotypeDefinitionResolvesBundleDefault() throws {
        let bundleURL = try makeBundle(
            definitions: [Self.definition(id: "D1"), Self.definition(id: "D2")],
            defaultID: "D1"
        )
    ...
    func testONTBarcodeNonBundleReferenceResolvesToNil() throws {
    ...
    // MARK: - Helpers (mirrors FastqGenotypingBundleReferenceTests)
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("FastqSiblingGenotypingBundleReferenceTests-\(UUID().uuidString)", isDirectory: true)
