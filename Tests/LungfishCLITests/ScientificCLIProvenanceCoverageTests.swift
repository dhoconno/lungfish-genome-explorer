...
final class ScientificCLIProvenanceCoverageTests: XCTestCase {
    ...
    func testDatabaseDownloadDelegatesToTheProvenanceEnforcingRegistryAPI() throws {
        ...
        XCTAssertEqual(section.components(separatedBy: "registry.downloadDatabase(name: name)").count - 1, 1)
        XCTAssertFalse(section.contains("MetagenomicsDatabaseInstaller("))
        XCTAssertFalse(section.contains("CanonicalMetagenomicsDatabaseInstallProvenanceWriter("))
        XCTAssertFalse(section.contains("installationRecipe"))
    }
