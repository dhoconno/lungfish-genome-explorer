...
final class FASTQIngestionProvenanceTests: XCTestCase {
    ...
    func testInPlaceFASTQIngestionMergesAdjacentImportedFASTQSidecar() async throws {
        let temp = FileManager.default.temporaryDirectory
            .appendingPathComponent("FASTQIngestionAdjacentSidecar-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: temp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: temp) }
        ...
        .argv(["lungfish-cli", "fastq", "materialize", "--output", importedFASTQ.path])
        .output(importedFASTQ, format: .fastq, role: .output)
        .step(
            ProvenanceStep(
                toolName: "lungfish-cli",
                toolVersion: "2026.05",
                argv: ["lungfish-cli", "fastq", "materialize", "--output", importedFASTQ.path],
                outputs: [try ProvenanceFileDescriptor.file(url: importedFASTQ, format: .fastq, role: .output)],
                exitStatus: 0,
                wallTimeSeconds: 1
            )
        )
        ...
        try ProvenanceWriter(signingProvider: nil).write(
            cliEnvelope,
            toSidecar: ProvenanceRecorder.fileSidecarURL(for: importedFASTQ)
        )
        ...
        let envelope = try XCTUnwrap(ProvenanceEnvelopeReader.load(from: bundleURL))
        XCTAssertEqual(envelope.steps.map(\.toolName), ["lungfish-cli", "clumpify.sh", "lungfish-app"])
        XCTAssertEqual(envelope.steps[0].outputs.map(\.path), [optimizedFASTQ.path])
        XCTAssertEqual(envelope.steps[0].outputs.first?.originPath, importedFASTQ.path)
        XCTAssertEqual(envelope.steps[0].durableReplayArgv?.last, optimizedFASTQ.path)
        XCTAssertFalse(envelope.files.contains { $0.role == .output && $0.path == importedFASTQ.path })
    }

    func testInPlaceFASTQIngestionRewritesImportedOutputOptionsAndReplayArgumentsToOptimizedPayload() async throws {
        let temp = FileManager.default.temporaryDirectory
            .appendingPathComponent("FASTQIngestionOptionRewrite-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: temp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: temp) }
        ...
        let sourceCLIOutput = temp.appendingPathComponent("cli-output.fastq")
        let importedFASTQ = bundleURL.appendingPathComponent("Sample.fastq")
        let optimizedFASTQ = bundleURL.appendingPathComponent("Sample.fastq.gz")
        try "@r\nACGT\n+\n!!!!\n".write(to: sourceCLIOutput, atomically: true, encoding: .utf8)
        try "@r\nACGT\n+\n!!!!\n".write(to: importedFASTQ, atomically: true, encoding: .utf8)
        ...
        .argv(["lungfish-cli", "fetch", "ncbi", "SRR123", "--output", sourceCLIOutput.path])
        .options(
            explicit: ["downloadOutput": .file(sourceCLIOutput)],
            defaults: ["defaultOutput": .string(sourceCLIOutput.path)],
            resolved: ["resolvedOutput": .file(sourceCLIOutput)]
        )
        .output(sourceCLIOutput, format: .fastq, role: .output)
        .step(
            ProvenanceStep(
                ...
                toolVersion: "2026.05",
                argv: ["lungfish-cli", "fetch", "ncbi", "SRR123", "--output", sourceCLIOutput.path],
                durableReplayArgv: ["lungfish-cli", "fetch", "ncbi", "SRR123", "--output", sourceCLIOutput.path],
                outputs: [try ProvenanceFileDescriptor.file(url: sourceCLIOutput, format: .fastq, role: .output)],
                exitStatus: 0,
                wallTimeSeconds: 1
            )
        ...
        .complete(
            ...
            endedAt: Date(timeIntervalSince1970: 51)
        )
        try ProvenanceWriter(signingProvider: nil).write(cliEnvelope, toSidecar: sourceSidecarURL)
        try GUIImportedProvenanceRehydrator.rehydrateImportedCopy(
            from: sourceCLIOutput,
        ...
        let envelope = try XCTUnwrap(ProvenanceEnvelopeReader.load(from: bundleURL))
        XCTAssertEqual(envelope.steps[0].durableReplayArgv?.last, optimizedFASTQ.path)
        XCTAssertEqual(envelope.durableReplayArgv?.last, optimizedFASTQ.path)
        XCTAssertEqual(envelope.options.explicit["downloadOutput"]?.fileValue?.path, optimizedFASTQ.path)
        XCTAssertEqual(envelope.options.defaults["defaultOutput"]?.stringValue, optimizedFASTQ.path)
        XCTAssertEqual(envelope.options.resolvedDefaults["resolvedOutput"]?.fileValue?.path, optimizedFASTQ.path)
    }
}
