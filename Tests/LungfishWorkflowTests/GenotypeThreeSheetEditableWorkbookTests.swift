...
final class GenotypeThreeSheetEditableWorkbookTests: XCTestCase {
    ...
    func testEvidencePreservationAndRetryNoOpRetainV2Receipts() throws {
        let fixture = try Fixture(); defer { fixture.remove() }
    ...
    private struct Fixture {
        ...
        func seed(rawSupport: Int? = 5, currentComment: String? = nil, h1Effective: String = "M1A", h1Pipeline: String? = "M1A") throws {
            let payload = GenotypeWorkbookPresentation.Payload(
                ...
                rows: [.init(id: "row-1", target: .init(kind: "row", locus: "MHC-A", genotype: "G1", stableClusterID: "cluster-1"), displayName: "G1", comment: nil, fillHex: nil, cells: [.init(sampleID: "scientific-S1", displayValue: 5, rawSupport: rawSupport, reviewEligible: true, fillHex: nil, comment: currentComment, review: nil)])],
        ...
        func edit(_ code: String) throws { try run("from openpyxl import load_workbook; from openpyxl.comments import Comment; from openpyxl.styles import PatternFill; w=load_workbook(p); " + code + "; w.save(p)") }
