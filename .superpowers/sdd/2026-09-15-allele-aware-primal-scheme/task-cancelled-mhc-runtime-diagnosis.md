...
| First family/discovery checkpoint reported in retained ledger | ~2026-09-16 02:18 | ~8h29m elapsed; this is a parent/ledger observation, not an exact SQL timer. |
...
| Cancellation | 11:10:03 / 11:10:53 | ~17h20m wall time; no root receipt/provenance. |
...
1. **Discovery/enumeration and history writes are the dominant early risk.** The retained 02:23 OS sample caught PID 24757 in Python `sqlite3_step` with 762/762 samples, including 725 `pread` samples and B-tree index/page movement. Physical footprint was 13.0 GiB and sampled peak 22.3 GiB. This is a real sample, not a retrospective peak; it does not identify one SQL statement or the run-wide maximum.
...
The cancelled run is old v1 (`7ca64e6`, textual `record_links` and 8 MiB page-cache implementation). Current matrix12 v2 (`74102b8`) changes only the three closure queries to physical integer `record_links_int`/`records.position`; it keeps all validation and durability checks. The synthetic benchmark in `task-history-scaling-benchmark.md` matched canonical IDs/snapshots and showed smaller v2 files and faster synthetic append/checkpoint cases, but it is not an end-to-end or full-MHC speed claim. It cannot retroactively improve the cancelled v1 run.
