...
- Initial un-attested legacy geometry is treated as presentation only. Tests now assert exact validated catalog/CSV identities, values and reviews, while preserving no-clobber and input checksum expectations.
...
- Genuine v1 coverage remains in `testThreeSheetMigrationRejectsUnreviewedV1EditsWithoutMutation`. The exact admitted legacy managed-state fixture now proves safe migration to schema 2 and CSV-derived evidence. Missing or unsupported editable-baseline versions fail closed without mutation.
...
- Scientific assertions read actual payload fields and workbook cells. Helpers now read actual `data_type`, sheet presence, retained calls, comments and manifest loci. Obsolete fabricated guide/audit/legacy-row measurements were removed.
...
| `testAbsentZeroSupportFalseNegativeUsesExactPortablePresentation` | same name | exact catalog identity, raw zero, FN review, provenance and portable OOXML semantics |
...
| `testCandidateUpdateRejectsMissingUnifiedPivotWithoutBundleMutation` | `testCandidateUpdateRebuildsMissingUnifiedPivotFromValidatedArtifacts` | validated artifact rebuild, exact candidate identities and original source checksum |
...
| `testDuplicateExactReviewTargetsFailClosedInEitherOrder` | same name | duplicate exact reviews withheld regardless of order; source records retained |
...
| `testExternalManagedStateVersionChangeRequiresReviewBeforeMigration` | `testExternalEditableBaselineVersionChangeFailsClosedWithoutMutation` | actual manifest-selected immutable export metadata mutation rejects/no-clobber |
...
| `testFullLengthMHCUpdateUsesSpeciesAgnosticBiologicalAlleleOrder` | same name | exact original `MHCAlleleDisplayOrder.lessThan` sequence retained; pending Task 6 |
...
| `testReviewBecomingInvalidRestoresPriorManagedPresentation` | same name | positive raw support invalidates FN promotion while retaining sidecar record; native-style restoration handed to Task 6 |
