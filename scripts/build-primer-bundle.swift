...
private let scriptToolName = "scripts/build-primer-bundle.swift"
private let scriptToolVersion = "1.1.0"
...
Behavior:
  ...
  - Writes manifest.json, primers.bed, optionally primers.fasta, and
    reproducibility provenance in PROVENANCE.md.
"""
...
func emitBundle(
    args: Arguments,
    stats: BEDStats,
    referenceHashes: [ReferenceVerification],
    argv: [String],
    startedAt: Date
) throws {
    ...
    if let fastaPath = args.fasta {
        let dest = outputURL.appendingPathComponent("primers.fasta")
        if fm.fileExists(atPath: dest.path) {
            try fm.removeItem(at: dest)
        }
        try fm.copyItem(at: URL(fileURLWithPath: fastaPath), to: dest)
        fastaDest = dest
    }

    let provenance = try makeProvenanceMarkdown(
        args: args,
        stats: stats,
...
func makeProvenanceMarkdown(
    ...
    let conda = env["CONDA_DEFAULT_ENV"] ?? "(not set)"
    let shellCommand = argv.map(shellQuote).joined(separator: " ")

    var options: [(String, String)] = [
        ("--name", args.name!),
        ("--display-name", args.displayName!),
        ("--description", args.description ?? "(not supplied)"),
        ("--organism", args.organism ?? "(not supplied)"),
        ("--canonical", args.canonical!),
        ("--equivalent", args.equivalents.isEmpty ? "(none)" : args.equivalents.joined(separator: ", ")),
        ("--bed", URL(fileURLWithPath: args.bed!).standardizedFileURL.path),
        ("--fasta", args.fasta.map { URL(fileURLWithPath: $0).standardizedFileURL.path } ?? "(not supplied)"),
        ("--source-url", args.sourceURL ?? "(not supplied)"),
        ("--source", args.source),
        ("--version", args.version),
    ...
    let optionRows = options
        .map { "| \($0.0) | \(tableCell($0.1)) |" }
        .joined(separator: "\n")
    let referenceRows = referenceHashes
        .map { "| \(tableCell($0.accession)) | \(tableCell($0.role)) | `\($0.sha256)` |" }
        .joined(separator: "\n")

    var inputRows = try [
        fileProvenanceRow(label: "Source BED", url: URL(fileURLWithPath: args.bed!)),
    ]
    if let fasta = args.fasta {
        inputRows.append(try fileProvenanceRow(label: "Source primer FASTA", url: URL(fileURLWithPath: fasta)))
    }

    var outputRows = try [
        fileProvenanceRow(label: "Bundle directory", url: outputURL, checksum: "(directory)"),
        fileProvenanceRow(label: "Manifest", url: manifestURL),
        fileProvenanceRow(label: "Bundled BED", url: bedDest),
    ]
    if let fastaDest {
        outputRows.append(try fileProvenanceRow(label: "Bundled primer FASTA", url: fastaDest))
    }
    ...
    | Field | Value |
    |---|---|
    | Tool/workflow | `\(scriptToolName)` |
    | Tool version | `\(scriptToolVersion)` |
    | Bundle schema version | `1` |
    | Bundle content version | `\(tableCell(args.version))` |
    | Started | `\(iso.string(from: startedAt))` |
    | Completed | `\(iso.string(from: completedAt))` |
    | Wall time | `\(String(format: "%.3f", elapsed)) seconds` |
    | Exit status | `0` after validation, NCBI equivalence checks, and file writes completed |
    | Runtime | `\(tableCell(ProcessInfo.processInfo.operatingSystemVersionString))` |
    | Conda environment | `\(tableCell(conda))` |
    | Container | `(not applicable)` |
    | Bundle created timestamp | `\(createdISO)` |
    ...
    | Field | Value |
    |---|---|
    | Primers | `\(stats.primerCount)` |
    | Amplicons | `\(stats.ampliconCount)` |

    ## Inputs

    | Label | Path | Size bytes | SHA-256 |
    |---|---|---:|---|
    \(inputRows.joined(separator: "\n"))

    ## Outputs

    | Label | Path | Size bytes | SHA-256 |
    |---|---|---:|---|
    \(outputRows.joined(separator: "\n"))
...
func fileProvenanceRow(label: String, url: URL, checksum: String? = nil) throws -> String {
    let path = url.standardizedFileURL.path
    let size = try fileSizeBytes(at: url)
    let digest = try checksum ?? fileSHA256(at: url)
    return "| \(tableCell(label)) | `\(tableCell(path))` | \(size) | `\(digest)` |"
}

func fileSizeBytes(at url: URL) throws -> UInt64 {
...
func run(_ argv: [String]) -> Int32 {
    ...
    do {
        ...
        var referenceHashes = [
            ReferenceVerification(accession: args.canonical!, role: "canonical", sha256: canonicalHash)
        ]
        ...
        for accession in args.equivalents {
            ...
            referenceHashes.append(ReferenceVerification(accession: accession, role: "equivalent", sha256: otherHash))
