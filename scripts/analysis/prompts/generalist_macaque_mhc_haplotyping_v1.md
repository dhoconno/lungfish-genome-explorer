...
- For MHC-B, expect MHC-B compound multi-gene haplotypes. A B haplotype may include several coherent Mamu-B, B17, B02/B10/B14/B16 pseudogene, I, or J-region genotypes. Prefer Mamu-B source-locus alleles as primary label anchors when they form repeated coherent patterns, but remember that one B haplotype can contain several Mamu-B source-locus alleles. Treat recurrent multi-Mamu-B patterns as one haplotype when they co-segregate; mine dominant co-segregating Mamu-B clusters before falling back to individual Mamu-B anchors. Then use B17/I/J and pseudogene signals as context unless they are uniquely paired with a haplotype. Do not define B haplotypes from only the two highest-read genes when broader repeated patterns are present; common B17/I/J signals may be shared background unless their pairing pattern is consistent.
- For MHC-DRB, expect DRB compound multi-gene haplotypes. A single DRB haplotype may include multiple coherent high-read DRB1, DRB3/4/5/6, or W-region genotypes, so do not treat every high-read DRB1 genotype as a separate haplotype. Seed DRB labels from recurrent whole-pattern evidence in homozygous or near-homozygous samples, then subtract those compound signatures from heterozygous samples. Before duplicating one matched DRB signature as a homozygous call, check residual DRB evidence for a second repeated compound pattern and call it separately when supported. A recurrent residual W-region, DRB3/4/5, or DRB6 pattern can define a DRB haplotype even when a DRB1 anchor is absent or weak.
