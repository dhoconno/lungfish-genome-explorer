...
| `GenotypeKnownAlleleDetailView.swift` | Allele identity, metadata, evidence, feature text, summaries, and GenBank/FASTA readers | The persistent detail subtree is updated in place from captured canonical fonts. Both read-only sequence readers scale from a stable monospaced baseline while preserving text, selection, scroll origin, reader identity, active tab, and scientific overview geometry. Labels wrap or expand, and current selection/detail state remains unchanged. |
| `GenotypeCandidateAlleleDetailView.swift` | Candidate identity, stable IDs, reference context, summaries, GenBank/FASTA readers, and sequence/difference explanations | The persistent detail subtree is updated in place from captured canonical fonts. Both read-only sequence readers scale from a stable monospaced baseline while preserving text, selection, scroll origin, reader identity, active tab, and scientific difference/reference geometry. Labels wrap or expand without recomputing candidate evidence. |
...
| `GenotypeResultViewController.swift` | Controller-generated Detail, Haplotype, Consumer, Anchor, and Audit/artifact text | Each generated stack receives a bounded typography observer, and every late rebuild refreshes its stack before returning so newly created fields immediately use the current preference. All ordinary fields update in place, wrap, expose complete accessibility values/tooltips, and preserve detail identity, selection state, and exact scroll origin. Persistent specialized detail views and SwiftUI hosting subtrees are excluded from the generic observer to prevent double scaling. |
