...
### Issues
1. **Annotation details not shown in Inspector** - Need annotation tab
2. **Color picker not updating on selection** - State binding issue
3. **Double-click/hover annotation popup** - Missing interaction handlers
4. **Sequence info on click** - Need sequence detail view

### Experts Assigned
- `frontend-developer` - Inspector annotation tab
- `ux-researcher` - Interaction design for popups
- `swift-expert` - SwiftUI state management
...
### Issues
1. **Zoom In/Out toolbar buttons** - UI addition
2. **Sequence height at low zoom** - Drawing calculation
3. **U→T display for RNA** - Alphabet conversion
4. **Base coloring threshold (~1% / 300bp)** - Zoom-dependent rendering

### Experts Assigned
- `frontend-developer` - Toolbar buttons
- `swift-expert` - Drawing optimization
...
### Issues
1. **RefSeq filter for NCBI Virus** - API parameter addition
2. **Search term autocomplete cache** - UserDefaults persistence
3. **Pathoplexus integration** - New database service

### Experts Assigned
- `api-designer` - Pathoplexus API research
