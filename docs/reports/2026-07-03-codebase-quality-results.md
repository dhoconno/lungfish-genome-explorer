...
| LungfishIO | **IN PROGRESS**. B1: `NaoMgsDatabase.swift` (2621L) -> 5 files (+Create/+Merge/+Summaries/+Queries) + `defer`-order tweak; 10 private->internal promotions; `logger`->`naoMgsDatabaseLogger` rename. B2: `AnnotationDatabase.swift` (1719L) -> 5 files (+Query/+Mutation/+Building/Record) + deduped the 4x 12-column row decoder into `decodeRecord` (all 4 originals diff-verified byte-equivalent modulo local names) + removed 1 orphan MARK; 3 private->internal promotions (`db`,`dbLogger`,`decodeRecord`). All pure relocation except the verified decoder dedup. | B1 ~+44; B2 net ~-13; B3 net ~+32 | see 02-io.md | B1/B2/B3 scoped-green: LungfishIOTests 2146 exec / 0 fail (2 env skips). Module-boundary full green-bar pending at IO end. B3: `ProjectUniversalSearchIndex.swift` (2484L) -> 4 files (+SQL/+Indexers/+Helpers) + 2 verified micro-edits (dup-recompute removal, redundant `String()` wrap). |
| LungfishWorkflow | - | - | - | - |
| LungfishKit | - | - | - | - |
| Leaf UI modules (9) | - | - | - | - |
| LungfishApp | - | - | - | - |
