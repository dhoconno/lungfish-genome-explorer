...
| LungfishCLI | **COMPLETE, CERTIFIED GREEN**. ALL 90 files audited (100% coverage). 15 big files (>=800L) solo + 8 infra (root/Options/Output/Support) + 68 Commands in 4 directory-sweep chunks. 1 committed batch plus post-review cleanup: **3 provably-safe dead-private-function removals** (ImportCommand.scanForFiles, FastqCommand.saveProvenance, FastqCommand.writeWorkflowRun) and one no-op wall-time cleanup. 2026-07-04 hardening centralized CLI provenance option merging, separated Markdup and scrub-human explicit options from resolved defaults, and made extract-contigs Core bundle provenance follow the manifest's actual uncompressed FASTA path when native compression is not used. CLI-clean: no LungfishKit import, no direct `GlobalOptions()` init, no persisted `.sam`, no `%s`-in-`String(format:)`, no forbidden GCD->MainActor hops, version literals untouched. | net ~ -50 lines (+ deferred splits) | see 07-cli.md | **CERTIFIED GREEN at module boundary**: full suite (ONT deadlock suite excluded) = 9531 XCTest / 0 fail + 487 swift-testing / 0 fail; ONT suite verified separately IN ISOLATION = 27/0. Combined = full clean baseline 9558 XCTest / 487 swift-testing, 0 failures. |
