...
| LungfishIO | **IN PROGRESS**. B1: `NaoMgsDatabase.swift` (2621L) -> 5 files (+Create/+Merge/+Summaries/+Queries) + `defer`-order tweak; 10 private->internal promotions; `logger`->`naoMgsDatabaseLogger` rename. B2: `AnnotationDatabase.swift` (1719L) -> 5 files (+Query/+Mutation/+Building/Record) + deduped the 4x 12-column row decoder into `decodeRecord` (all 4 originals diff-verified byte-equivalent modulo local names) + removed 1 orphan MARK; 3 private->internal promotions (`db`,`dbLogger`,`decodeRecord`). All pure relocation except the verified decoder dedup. | batches net roughly flat (splits add extension boilerplate; dedup/dead-code removals subtract) | see 02-io.md | All batches scoped-green: LungfishIOTests 2146 exec / 0 fail (2 env skips). Batches committed: B1 NaoMgsDatabase split; B2 AnnotationDatabase dedup+split; B3 ProjectUniversalSearchIndex split+2 edits; B4 GenotypeHaplotypeAnalysis split+edit; B5 MSABundle dedup+tighten+split; B6 PhylogeneticTreeBundle split+inline; B7 FASTQDerivatives split+edit; B8 TwelveSAmplicon split; cluster-edits (5 dead-code/reuse removals). Full IO-boundary green-bar pending. |
| LungfishWorkflow | - | - | - | - |
| LungfishKit | - | - | - | - |
| Leaf UI modules (9) | - | - | - | - |
| LungfishApp | - | - | - | - |
