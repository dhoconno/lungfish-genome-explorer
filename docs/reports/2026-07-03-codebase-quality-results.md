...
`git diff main...worktree-fable-codebase-quality`; per-module rationale + everything
...
| LungfishCLI | **COMPLETE, CERTIFIED GREEN**. ALL 90 files audited (100% coverage — reconciled against `find`: coverage ledger in 07-cli.md shows audited == total for every directory row; 90 audited / 2 applied / 88 clean / 0 deferred-unaudited). 15 big files (>=800L) solo + 8 infra (root/Options/Output/Support) + 68 Commands in 4 directory-sweep chunks. 1 committed batch, **2 provably-safe dead-private-function removals** (ImportCommand.scanForFiles, FastqCommand.saveProvenance). Statement-level clean (same pattern as all prior modules); value beyond the 2 removals is in DEFERRED command-file splits (catalogued in 07-cli.md). CLI-clean: no LungfishKit import, no direct `GlobalOptions()` init, no persisted `.sam`, no `%s`-in-`String(format:)`, no forbidden GCD->MainActor hops, version literals untouched. VERIFY-EVERY-CLAIM rejected the recurring auditor misfire (a private helper called at even one site is LIVE, not dead — "8+2 dead statics" were all live single-caller helpers) and correctly kept cross-type byte-identical helpers (not dedup-able). | net ~ -41 lines (+ deferred splits) | see 07-cli.md | **CERTIFIED GREEN at module boundary**: full suite (ONT deadlock suite excluded) = 9531 XCTest / 0 fail + 487 swift-testing / 0 fail; ONT suite verified separately IN ISOLATION = 27/0. Combined = full clean baseline 9558 XCTest / 487 swift-testing, 0 failures. |
