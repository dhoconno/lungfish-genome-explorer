...
| NEW-08 | P1 | Cancelled EsViritu operation stays active after Cancel All (tool processes gone) | open | lane | | live GUI |
| NEW-09 | P1 | Cancel Operations and Quit never quits (discarded terminateNow) | fixed | orchestrator | 840f45af4 | QuitWithRunningOperationsTests fail without fix |
| NEW-10 | P2 | After an Inspector delete (now persisted), the annotation drawer keeps showing the deleted row and the old count until the bundle is reopened | open | orchestrator | - | live GUI 2026-09-24; routing the reload through reloadReferenceBundleAfterAnnotationTrackMutation did not discriminate in tests and destabilized testRapidSuccessiveUpdates, so reverted |
