...
| TST-01 | P0 | Pre-push unit tier is red on HEAD: 117 failures plus 1 indefinite hang | partially fixed: a full clean unit-tier run at package handoff shows 83 xctest + 2 swift-testing failures remaining (down from 117; the 1 indefinite hang is now excluded and opened as a P1-B product-bug investigation, not fixed). TST-03 fully closed; TST-04 fixed for the ~15 files/classes actually verified (SRAServicePathTests, WorkflowEngineLaunchTests, LocalWorkflowRuntimeEvidenceTests, ManagedWorkflowRunnerPathTests, BuildDbCommandTests, MarkdupCommandTests, BuildDbCommandMarkdupTests); most of the remaining ~83 failures are the same `.lungfish`-vs-`.lungfish-stable` root cause in files not yet migrated (Minimap2ResultSidecarTests, MappingSummaryBuilderTests, OrientPipelineTests, TaxTriagePipeline*Tests, ViralVariantCallingPipelineTests, NativeBundleBuilder*Tests, RecipeEngineTests, FASTQDerivativeServiceProvenanceTests, and others — see final report for the full list). group 3 (runModal, .superpowers/, palette) fixed. Groups 4/5, AppleContainerRuntimeIntegrationTests, and the DocumentationAccuracyTests/GUIRegressionTests/PackToolManifestConsistencyTests/PluginPackRegistryTests drift items not addressed | P0-A | 18b87a387, b6c6cddfb, 62d9f3a76, 7ef2b46eb, 82133f4a6 | unit-gate-run4 (45-min budget, no hang, no timeout): 13694 xctest selected/13626 executed/83 failed, 570 swift-testing/2 failed; full list in final report |
...
| TST-04 | P1 | Stable-namespace change broke about 75 tests that hard-code `.lungfish` fake homes, and tests cannot inject an | partially fixed: made CoreToolLocator/SamtoolsLocator/SRAService/WorkflowEngineLaunch/BaseWorkflowRunner identity-injectable (appIdentity param, default .current) and migrated the classes that actually exercise them (SRAServicePathTests, WorkflowEngineLaunchTests, LocalWorkflowRuntimeEvidenceTests, ManagedWorkflowRunnerPathTests, BuildDbCommandTests, MarkdupCommandTests, BuildDbCommandMarkdupTests, MetadataPresetStoreTests). A clean unit-tier run still shows ~50-60 further failures from the same root cause in files not yet migrated: Minimap2ResultSidecarTests, MappingSummaryBuilderTests, OrientPipelineTests, TaxTriagePipeline(Provenance)Tests, ViralVariantCallingPipelineTests, NativeBundleBuilder(OffMain\|Provenance)Tests, ONTBAMImportMaterializerTests, DemultiplexingPipelineTests, RecipeEngineTests, PBAAClusteringPipelineTests, FASTQDerivativeServiceProvenanceTests, MaterializationPipelineProvenanceTests, ScientificFASTQProvenancePolicyTests, SamtoolsLocatorTests (the test class itself, not just the resolver), BAMImportServiceTests, ImportFastqCommandTests, TreeCommandTests, CondaPacksCommandTests, EsVirituDatabaseManagerTests, DependencyPlannerTests, and others (81 `.lungfish/conda` occurrences remained across ~65 files at package handoff, per an earlier grep). The fix pattern is mechanical (add `appIdentity: .preview` at each `CoreToolLocator`/`ManagedStorageConfigStore` call site touching a `.lungfish` fixture) but the volume exceeded this package's remaining time | P0-A | b6c6cddfb, 82133f4a6 | SRAServicePathTests (9/9), WorkflowEngineLaunchTests (9/9), LocalWorkflowRuntimeEvidenceTests (2/2), ManagedWorkflowRunnerPathTests (5/5), BuildDbCommandTests, MarkdupCommandTests, BuildDbCommandMarkdupTests pass; unit-gate-run4 shows the remaining scope |
