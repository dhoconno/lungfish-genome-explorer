...
| NEW-08 | P1 | Cancelled EsViritu operation stays active after Cancel All (tool processes gone) | fixed | lane+orchestrator | 70356de90..HEAD | root cause live: worker stuck in openat (NEW-11). detectToolVersion rethrows cancellation; cancel() forces Cancelled after 10 s but keeps the bundle lock until the worker returns; 5 grace tests |
