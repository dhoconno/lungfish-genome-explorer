...
| TST-01 | P0 | Pre-push unit tier is red on HEAD: 117 failures plus 1 indefinite hang | fixed | stabilization+orchestrator | 26744cec9 | GATE PASS 13,907 XCTest 0 failures (D8 quarantine of 5) |
...
| REL-03 | P1 | GPL-2.0 Linux kernel shipped without notice or source offer; THIRD-PARTY-NOTICES stale and not bundled | fixed | P8-A | a9f982c67, 89d54d1bf | generated notices + bundled + smoke gate. D16: owner email kept in source offer, all license/source URLs pinned to Package.resolved commits (generation fails on branch URLs), zstd BSD election stated |
| REL-04 | P1 | No rollback or yank path for a bad Sparkle release; the floor gate blocks the obvious one | fixed | P8-A | 267e82746, d4820e20c, 1fbd7c4fa | D17: yank is plan-only by default. --execute (typed-version confirmation) removes the item, optionally restores the last good item, keeps the floor via an lge:yanked marker, marks the release prerelease `Yanked:`. Floor --yank mode removed. Never run against real GitHub |
...
| GEN-02 | P0 | `minimumMatches: 1` plus a count-only match rule reports homozygotes as heterozygotes (DQ M2/M2 as "M2 / M6",  | fixed | lane(D11) | ce6983282 | independent-evidence rule; 28-genotype golden test: DQ/DR 28/28, DP identical definitions reported as M4|M7 ambiguity, no wrong calls; callingRulesVersion 2 |
...
| GEN-04 | P1 | Reads tied across alleles are credited in full to each allele with no ambiguity marker. minimap2 `-N 5` makes  | fixed | lane(D12) | bbad8c32e,ff013ae39 | identical refs (incl. revcomp) collapsed with ambiguous_with; e1 fixture 1 row x 20 reads |
| GEN-05 | P1 | "Locus %" uses three different locus groupings (pipeline haplotype filter, matrix "Viewed Locus", evidence pan | fixed | lane(D13) | 08e576129 | GenotypeLocusDenominator per source locus used by matrix, evidence pane, caller dropout and Excel filter; worked example 50% everywhere; callingRulesVersion 3 |
| GEN-06 | P1 | "Minimum percent" means within-sample read fraction for known alleles but fraction of animals for candidate ro | fixed | lane(D14) | 08e576129 | Min % per-sample read fraction for known and candidate; new Seen in >= N% of animals control; 30-animal example passes |
...
| GEN-10 | P2 | Full-length ONT: a zero-SNP hit is a known call regardless of indel size, with no indel count in the call | fixed | lane(D15) | ff013ae39 | known calls keep indels, add indel_bases + review_flag |
...
| NEW-02 | P2 | Sidebar watcher misses CLI changes in newly created project | fixed | lane(D20) | 9d115a159 | coalesced backstop rescan on activation/key window; 5 tests |
...
| DS-02 | P2 | Small contigs: padded fetch window spreads sample over whole contig; in-view share small | fixed | lane(D9) | b9cedbb36,ffae6287e | superseded by the 500x depth cap; verified live (65,424 of ~671,705 reads, cap override 100x refetches) |
| TST-15 | P2 | Pre-existing Genotype Excel contract conflict + InvalidTransition flake (5 tests) | fixed | lane(D8) | fe44e33a0 | D8 decided; five tests reconciled to exact-notes + evidence column; KNOWN_PREEXISTING_FAILURES empty |
...
| NEW-10 | P2 | After an Inspector delete, the drawer keeps the old row and the operation stays at 0% | fixed | via NEW-11 | 20701c06d | live: drawer shows 21 immediately after Inspector delete |
| NEW-11 | P1 | `NoFollowFileSystem.openDirectoryHierarchy` opens every ancestor from `/`, so writes under ~/Desktop, ~/Documents or ~/Downloads need the folder-level TCC grant even when the project was opened by user intent. Without it, provenance writes, `ProjectTempDirectory.create` and the FSEvents watcher setup block forever in `openat` (or fail with EPERM) while plain full-path opens keep working | fixed | lane(D18) | 20701c06d | walk anchored at the .lungfish project/bundle; live: Desktop-project delete completes, provenance written, 0 threads in openat |
| D10 | P2 | One slider control with direct numeric entry everywhere (owner request 2026-09-24) | partial | lane+orchestrator | c3603ad1a,43a961b3f,ffa93316c | 7 raw sliders migrated to NumericSliderField; pre-push ratchet scripts/ratchets/shared-slider-control.sh; ReadStyleSection pending the D9 depth-cap lane |
| REL-06 | P2 | build-notarized-dmg.sh runs the app smoke only for Stable (~line 1491) while release-candidate-receipt.py (~546) demands smoke evidence for every channel when appSmokeRequired is true; re-enabling D6 would break every `package preview` | open (after release) | release lane | - | pinned by expectedFailure test_preview_package_binds_app_smoke_when_policy_requires_it; fix: drop the Stable-only guard and pass the channel through app_smoke_gate.py (and gate_fixtures.make_app_smoke_fixture) |
| REL-07 | P3 | gate_evidence.py records dependencySourcePath as absolute (~920) and compares it with the verifying checkout root (~875), so re-verifying a receipt from a tag worktree (nightly tagged_publication_state) always fails | open (after release) | release lane | - | does not affect package/publish at HEAD; pinned by expectedFailure test_completed_publication_integrates_later_agent_work_and_prepares_next_calver; fix: compare the repo-relative lock path and bytes |
