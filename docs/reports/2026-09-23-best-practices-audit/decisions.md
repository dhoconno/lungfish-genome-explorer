...
| D8 | **OPEN, owner to decide:** the Genotype Excel Filtered-sheet comment contract. Five tests expect comments wrapped as "Evidence: display=…, raw support=…", while the green contract test `GenotypeExcelExportServiceTests.testMatrixCommentsAreExactUserNotesAndReviewsRemainPureFormatting` requires exact user notes. Both fail or pass that way at the pre-audit base. Until decided, the five are quarantined in `scripts/full-suite-gate.sh` (`KNOWN_PREEXISTING_FAILURES`, ledger TST-15) so the release gate stays usable | Team (interim) | Remove entries once decided |
