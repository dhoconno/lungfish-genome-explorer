...
The first candidate and exact reuse passed, but a separate new-candidate experiment exposed Xcode archive-action cleanup: its archive-specific intermediate subtree was recreated, all compiler arguments matched, and external C/Swift targets rebuilt. Packaging therefore uses the supported native Release `build` action, then validates/copies the app and symbols into its retained archive layout. The 15m8 archive-action measurement is historical. The corrected native package at `f4f0d78caecb346dd9e02020c85512516384fba7` passed in 96.224 seconds, including fresh compact source gates and artifact qualification. It reused compiler products across commits: Xcode took 3.358 seconds with zero Swift/C compiles and retained its build database. Cold compilation took 665.871 seconds; its first package failed portability until the copied app/CLI received the same deterministic strip step as an archive. The sanitizer now uses one binary read per executable and writes only changed bytes. The failed attempt remains diagnostic evidence, not a candidate.
