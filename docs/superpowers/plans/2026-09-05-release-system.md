...
- [ ] Verify current entrypoint and resources; add tests for native target dependency/copy and stable executable product.
- [ ] Convert CLI module to library target and add wrapper executable target; add native Xcode CLI target linking the library product.
- [ ] Make archive build the CLI as a dependency and copy its product; remove the second SwiftPM Release build.
- [ ] Split compiler cache schema from policy hashes using explicit compiler-recipe inputs; keep receipt hashes comprehensive.
...
- [ ] Define catalog schema, exact selection/empty/ambiguous validation and profile composition tests.
- [ ] Preserve all suites in documented collections; keep native AppKit and XCUITest in UI diagnostics, expensive exhaustive release transactions in maintainer/extended collection.
- [ ] Delegate executions to existing evidence parser, preserving discovered/completed/skip/exit accounting. Avoid independent concurrent Swift builds.
- [ ] Provide compatibility for existing tier commands and source checks through behavior-based catalog assertions.
...
- [ ] Add loader/migration tests for v1/v2, unsafe paths, invalid selectors and cross-repository mismatch.
- [ ] Implement explicit machine configuration and setup/readiness distinction; do not silently perform credential import/unlock/rotation.
- [ ] Validate Sparkle selected public key against candidate public key; verify disposable signatures independently.
- [ ] Supervise credentialed process groups with phase budgets, closed stdin and safe logs. A timeout is a failure/recoverable state, never a permission bypass.
- [ ] Persist notarization ID/artifact hash before bounded polling; retry reuses submission. Test timeout, descendants, invalid result and ambiguous upload states with fake tools.
- [ ] Document unavoidable one-time OS provisioning and the precise limitations of unattended readiness.
...
- [ ] Add behavior tests for accepted/rejected fork metadata, all isolated paths, upstream compatibility and embedded/enclosing mismatch.
- [ ] Implement strict identity parsing/resolution and legacy migration policy.
- [ ] Route fork product URLs/help identity through validated metadata; preserve scientific format names.
- [ ] Validate app and embedded/copied CLI identity with packaged fixtures and unchanged upstream identity tests.
...
- [ ] Add committed public identity: repository, Sparkle key, optional fork namespace and channel names/IDs/feeds. Validate it before build and bind it in candidate evidence.
- [ ] Add public configuration commands for reviewable fork identity and private machine selectors; retain existing default profile compatibility.
- [ ] Separate fast Debug checks from optional full portable Debug diagnostics; package is optimized/headless and publish uses exact candidate.
- [ ] Replace unconditional exhaustive release-maintainer tests with declared routine source checks and actual artifact checks; retain extended diagnostics.
- [ ] Reuse an existing candidate only after exact source/policy/artifact/evidence verification. Gate reuse must match all declared runtime/test-binary inputs or be disabled.
- [ ] Add named phase timings and persistent failure/recovery metrics with redacted arguments.
- [ ] Integrate credential selectors, bounded signing/notary, public key verification and fork metadata into artifact creation/publication.
...
- [ ] Run independent build-graph, test-policy, security and fork-runtime reviews; resolve correctness findings.
- [ ] Update every operator front door and remove stale claims that all tests/UI are mandatory for release.
- [ ] Run changed-area behavioral tests and one exhaustive release-maintainer validation for the redesigned machinery; retain all failures and resolutions.
...
- [ ] Verify fork initialization in a temporary repository with fake selectors/public key, without using upstream private credentials.
