...
- [ ] **Step 1: Write failing classifier and discovery tests**
...
- [ ] **Step 2: Run the tests and confirm the BAM assertions fail**
...
- [ ] **Step 3: Add the minimal shared classifier and use it during discovery**
...
- [ ] **Step 5: Commit the source-recognition slice**
...
For FASTQ, return the original pair and no steps. For BAM, call the shared `convertBAMToSingleFASTQ` helper into `workspace/<sample>.fastq`, compress it with managed `pigz -p <threads> -c`, verify non-empty `workspace/<sample>.fastq.gz`, and return a single-end processing pair. Capture a samtools conversion step, a Lungfish merge step, and a pigz compression step with exact arguments, versions, timing, status, stderr, input/output descriptors, and the original BAM as the scientific input.

- [ ] **Step 4: Insert materialization before recipe execution**
...
- [ ] **Step 5: Run materializer and importer tests**

Run: `swift test --filter 'ONTBAMImportMaterializerTests|FASTQBatchImporterTests'`

Expected: all selected tests pass.

- [ ] **Step 6: Commit BAM materialization**
...
Import a small BAM with fake managed samtools/pigz executables and assert the completed bundle contains:
...
- [ ] **Step 2: Run tests and confirm the provenance format is wrong**
...
- [ ] **Step 3: Thread materialization steps into the provenance writer**
...
- [ ] **Step 3: Add the narrow routing changes**
...
- Preserve ordinary BAM alignment import when the user chooses the dedicated BAM/CRAM alignment card; only the sequencing-read action and direct project read drop use ONT BAM import.
...
- [ ] **Step 2: Run the CLI integration test with a generated BAM fixture**

Run: `swift test --filter ONTBAMImportEndToEndTests`

Expected: the test creates a temporary unmapped BAM with managed samtools, imports it, and verifies one `.lungfishfastq` bundle whose primary payload is `fastq.gz` and whose final provenance input is the BAM.
...
- [ ] **Step 5: Run the broader build and tests**

Run: `swift build && swift test --filter FASTQ`

Expected: build succeeds and all FASTQ-selected tests pass.
