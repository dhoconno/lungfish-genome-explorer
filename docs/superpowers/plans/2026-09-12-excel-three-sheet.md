...
- [ ] Replace obsolete scan counters with behavior/no-op assertions. Route genuinely absent planned style or biological row-order behavior to Task6 with exact test names and original expected values; do not weaken those assertions or fake old metrics. Report behavioral failures immediately, implement only scoped fixes after confirming the cause.
...
- [ ] Preserve existing analyst matrix styles in both roles, not only fills: text color, border, bold/italic with explicit false overrides, and row/column/cell precedence. Extend captured snapshot/projection/payload with backward-compatible optional style fields as necessary; use resolved LGE rendered styles for filtered parity. Test explicit false overrides and border/font propagation without changing counts, Note targets, or review eligibility.
- [ ] Run one integrated affected suite with `swift test --jobs 6 --filter 'GenotypeWorkbookPresentationTests|GenotypeThreeSheetEditableWorkbookTests|GenotypeEditableWorkbookTests|GenotypePivotFilteredCopyTests|GenotypeWorkbookRevisionServiceTests|GenotypeViewportExcelExportTests|GenotypeReviewedHaplotypeInferenceTests|GenotypeEffectiveHaplotypeProjectionTests|GenotypeCurrentWorkbookInputFingerprintTests|GenotypeCurrentWorkbookSyncCoordinatorTests|GenotypeExcelDialogBehaviorTests|GenotypeAuditTimelineExpansionTests|GenotypeReviewedHaplotypeEvidenceTests|GenotypeMatrixReviewCapabilityTests|GenotypeAnnotationStoreTests|GenotypeResultViewportMatrixReviewTests'`, adding the new shared-eligibility helper suite and exact style-capture tests introduced in Tasks5/6. Record counts, skips, failures, exact commands and limitations in QA doc. No passing claim if a required invariant remains unverified.
