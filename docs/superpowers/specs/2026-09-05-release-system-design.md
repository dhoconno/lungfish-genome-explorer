...
2. Use a single incremental Xcode Release `build` dependency graph for GUI and CLI, then retain a compatible internal `.xcarchive` layout containing app, dSYMs and metadata. Xcode `archive` recreates its archive intermediates and therefore is not the package compilation action. Keep LungfishCLI as a library module with a shared thin executable entrypoint, preserving command behavior and tests. Retain one SwiftPM graph for incremental Debug. Compiler cache identity contains compiler inputs, not release-policy/signing changes; artifact receipts still bind all policy and source inputs.
...
8. Record named phase timings, cache hits, commands with nonsecret selectors, exits and recovery state. Measure warm Debug, first/warm Release builds, policy-only changes and exact candidate retries. Do not claim a cold Debug benchmark when existing intermediates were used. Report measured results, not promises about all-core CPU use.
