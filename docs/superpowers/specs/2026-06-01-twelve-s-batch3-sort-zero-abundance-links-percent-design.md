...
2. **Pass B (abundance reassignment).** After Pass A, compute per-species exact-read totals from `countsByTarget` (map each targetID → species via the target table's `scientificNameKey`, sum across samples). Then for each ambiguous sequence with candidates spanning multiple species:
   ...
   - **Policy margin (locked): strict plurality, any nonzero lead wins.** The winner is the candidate species whose unambiguous exact-read total is strictly greater than every other candidate's, and is `> 0`. No ratio threshold: human 1000 vs pan 0 → human; human 1000 vs pan 3 → human; 500 vs 500 (exact tie) or all-zero → stays ambiguous/unresolved.
