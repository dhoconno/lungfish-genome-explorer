...
### Optional concrete secondary-product certificates

Retained A1 pair evidence motivates an independently reviewed opt-in `ordered-disjoint-concrete-designated-sites/v1` secondary policy. Keep existing exact-secondary and reject-secondary policies and defaults unchanged. A new allowance requires two complete same-row configurations using their own four selected site occurrences, all full footprints concrete and independently supported by the existing terminal-hit predicate, including internal partners. Outer hit sequences, orientations and both endpoints must match those occurrences; complete local products must be ordered and disjoint. All other specificity, uncertainty, dimer, overlap and pool constraints remain effective.

New secondary witnesses retain the full four-site certificate and policy identity, separate from same-configuration intended witnesses, and add zero exact coverage. Fresh audits reconstruct certificates from authoritative raw rows and compare saved allowances. Intended and secondary policy choices remain explicit independent axes. This is an optional interpretation to evaluate, not a default change or a claim of joint feasibility or amplification success.
