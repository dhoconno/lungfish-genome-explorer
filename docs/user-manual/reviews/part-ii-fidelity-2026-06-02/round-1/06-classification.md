...
4. The viewport is a single-import split view (detail pane plus taxonomy table) with columns Sample, Taxon, Hits, Unique Reads, Refs, plus per-sample metadata columns, metrics, and miniBAM evidence cards when alignment data is available. There is no time-series chart and, after 2026-07-05 hardening, no per-accession coverage sparkline in the shipped NAO-MGS viewport.
