...
**Maximum displayed depth.** Sets how deep the drawn pile may get before LGE starts sampling reads. It defaults to 500x, and you can drag the slider or type a value from 50x to 5,000x in steps of 50. LGE splits each window into stretches of about a thousand bases, and any stretch deeper than the setting is sampled down to about that depth, while every other stretch shows every read. So a shallow flank next to a very deep amplicon keeps all of its reads instead of being thinned along with the amplicon. Lower the setting on an older machine when deep windows feel slow to redraw, and raise it when you want to see more of a deep target's reads at once. It is a different limit from the two-million-read ceiling the **Load all** button reaches, which is described under Reading the results below.
...
When part of a window is deeper than the maximum displayed depth, LGE samples that part and posts a banner over the track reading "Showing 72,087 of ~2,050,000 reads · regions above 500x are sampled to about 500x, all other regions show every read · depth, coverage and consensus use all reads", with a **Load all** button beside it. Those numbers are an example taken from a much deeper dataset. The total carries a "~" because it is estimated from the sample. When no part of the window is deeper than the setting, no banner appears and every read is drawn. Pressing **Load all** shows every read in that window up to a ceiling of two million reads.

The last part of that banner is the one to hold on to. The coverage curve, the depth numbers, and the consensus row are computed from every read in the BAM by separate queries that never see the depth setting. Only the drawn read rows are sampled. So a sampled window still tells you the truth about depth and strand balance. The sample is drawn with a fixed seed, which means the same reads appear every time you redraw and a read you selected is still there when you come back. Paired reads are kept or dropped together within a sampled stretch.
