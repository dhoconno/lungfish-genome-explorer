...
3. Read the **Sample** section. It holds a text field carrying a sample name worked out from the file name, and under it one line describing the reads. That line is a report rather than a control. LGE checks the start of the file and the bundle's import record to choose it. **Interleaved paired-end reads** means every read sits next to its mate, and EsViritu runs them as pairs. **Mixed paired and merged reads (run as single-end)** means the bundle holds pairs plus reads that were merged or lost their mate, as the VSP2 import recipe produces. EsViritu has no mode for that mix, so LGE runs every read as single-end and records why in the run's provenance. **Paired-end reads** means two separate mate files, and **Single-end reads** means no read has a mate. If it says Single-end when you expected a pair, close the dialog and fix the selection in the sidebar rather than looking for a control to change it here. A correct selection is the one bundle row holding both mate files, not two separate single-file bundles clicked together.
