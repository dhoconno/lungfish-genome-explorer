...
A bit score has no fixed good or bad value, because it grows with the length of the read. Its use is comparing the hits for one read against each other. Expand a read's row and compare its hits. A 76-base read that matches perfectly scores about 140. If its top hit scores 140 and the second 139, the read fits both records almost equally well, and the organism named on the top row is barely preferred. If the top hit scores 140 and the second 100, the top hit is a clear winner. Do not compare bit scores between reads of different lengths.
...
A verification you can rely on has a Supported word, and per-read rows that name the classifier's organism with identity and coverage in the high nineties and e-values far below `1e-10`. The worked example came back that way. Its summary bar read `BLAST for Simplexvirus humanalpha1: 20 supporting, 0 contradicting (20 reads)`, followed by the word Supported, and the top hits named Human alphaherpesvirus 1. With every one of 20 reads agreeing, the share rests on all the reads sent, so leave the slider at 20 for any verification whose answer you intend to use.
...
| TaxTriage | **BLAST Verify** in the action bar, or right-click **Verify with BLAST...** | The longest reads, five or a quarter of the request, whichever is smaller, then a random fill, searched against `core_nt` | Confidence word |
| NAO-MGS | **BLAST Verify** in the action bar, or right-click **BLAST 20 Reads**, **BLAST 50 Reads**, or **BLAST All N Reads** (N being the taxon's unique read count, with at most 50 sent) | Reads spread across the four quarters of the reference genome, searched against only that taxon's own records in `core_nt`, so the word leans toward Supported | Confidence word |
