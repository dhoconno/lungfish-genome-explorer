...
Treat the word as a strong second signal rather than a final answer. NCBI's collection is broad but not complete, the default of twenty reads shows what most of a taxon's reads are but can miss a small minority, and outside the taxonomy viewport a read can match a close relative well enough to count as agreement. When a classification surprises you, verify it before you act on it, and read the per-read rows rather than the summary word alone.
...
3. Read the popover that opens over the middle of the sunburst chart, the ring chart beside the table where each wedge is one taxon. A popover is a small panel that closes when you click anywhere outside it. Its title reads `Verify "Simplexvirus humanalpha1" via NCBI BLAST`. The line under the slider names the database the search will use, `nt` from this viewport, and warns that the reads leave the app for NCBI.
...
| **Error** | NCBI's answer held no usable entry for the read, either none at all or one LGE could not read, and the Organism column says "No BLAST result" |

An Error read is a failed search, not evidence against the taxon. The rest of the batch is still judged, and only a run in which every read comes back Error fails as a whole.
...
The summary bar reads `BLAST for <taxon>: <n> supporting, <n> contradicting (<n> reads)`, then a row of ten symbols and the confidence word. When some reads came back Error, the bar adds their count before the brackets, as in `2 with no BLAST result`. Only Verified reads count toward the supporting and contradicting counts.

A Verified read is **supporting** when its top hit agrees with the taxon you selected, and **contradicting** when it names something else. From the taxonomy viewport and the command line, LGE judges agreement by [taxonomy ID](../../GLOSSARY.md#taxonomy-id) first. A hit agrees when its taxonomy ID falls inside the selected taxon's [clade](../../GLOSSARY.md#clade), the taxon with everything below it in the Kraken 2 report. NCBI often files a record under a strain's own taxonomy ID, which the report does not list, and then the hit agrees when its organism name contains the whole name of a member of that clade. That is how a hit that NCBI names Human alphaherpesvirus 1 supports *Simplexvirus humanalpha1*, the species name Kraken 2 and Bracken use for the same virus. Any other species contradicts, even one in the same genus, so a hit to Human alphaherpesvirus 2 contradicts HSV-1, and so does one to Human gammaherpesvirus 4, a different herpesvirus.

The other viewports do not hand LGE the classifier's taxonomy, and a few hits carry no taxonomy ID, so those hits are judged by name alone. A hit then agrees when its organism name contains the selected name, or when the first words of the two names match, which for a species name is the genus. That older rule counts a different species in the same genus as supporting, and it does not know that many virus names start with a host, such as Human. Check the Organism column when the exact species matters. Ambiguous, Unverified, and Error reads count as neither supporting nor contradicting.
...
A phrase reading `<n> with conflicting organisms` can appear beside the word. It counts reads whose five hits disagree about the organism. From the taxonomy viewport, when every hit carries a taxonomy ID, a read counts when its hits mix the selected taxon's genus with organisms outside that genus, so hits to HSV-1 and HSV-2 together do not count, but hits to HSV-1 and Human gammaherpesvirus 4 do. Otherwise LGE compares the first word of each organism name, so viruses whose names start with the same host word, such as Human, count as one genus there. A handful is normal, but when it reaches about a quarter of the reads you sent, the reads themselves are ambiguous. That usually means they come from a conserved region, a stretch of sequence so little changed over evolutionary time that several genera still share it. Such reads cannot tell the organisms apart, which is a different problem from the classifier being wrong.
...
LGE saves every finished verification from the taxonomy viewport inside the Kraken 2 result folder, in a `blast-verifications` folder. Each verification is one file named with the taxonomy ID and the time it finished, with a [provenance](../../GLOSSARY.md#provenance) record beside it. When you select that taxon again, even after quitting LGE, its newest verification waits in the drawer's BLAST Results tab, and clicking **BLAST Results** shows it. The other viewports do not save verifications, and their Operations Panel rows keep only the two counts, so click **Export** there before you move on if you need the per-read rows.

## What good looks like

Read the summary word and the per-read rows together, because either alone can mislead you.

A verification you can rely on has a Supported word, and per-read rows that name the classifier's organism with identity and coverage in the high nineties and e-values far below `1e-10`. The worked example came back that way. Its summary bar read `BLAST for Simplexvirus humanalpha1: 20 supporting, 0 contradicting`, with all ten circles filled, the word Supported, and the phrase `1 with conflicting organisms`. The rows' top hits named Human alphaherpesvirus 1 at 100.0 percent identity, with e-values of `1e-29` and `4e-29` and bit scores of 140 and 141. This example was run with LGE 2026.9.43, which judged that phrase by the first word of each name alone, so for one read the up to five hits BLAST returned included a name whose first word differed from the rest. Its top hit still agreed with HSV-1, so it still counts as supporting, and one read in 20 is the handful that is normal. With every one of 20 reads agreeing, the share rests on all the reads sent, so leave the slider at 20 for any verification whose answer you intend to use.
...
| TaxTriage | **BLAST Verify** in the action bar, or right-click **Verify with BLAST...**. Both open the popover | The longest reads, five or a quarter of the request, whichever is smaller, then a random fill, searched against `core_nt` | Confidence word |
