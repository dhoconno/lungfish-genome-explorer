...
4. Enter an accession or free-text term, then narrow with the filter fields: Country, Host, Clade, Lineage, Nucleotide Mutations, Amino Acid Mutations, Collection Date, Sequence Length, and INSDC Source.

Clade and Lineage are separate fields. Clade takes a clade label such as `IIb`, Lineage a lineage designation such as `B.1`. The two mutation fields are separate too. Nucleotide Mutations takes comma-separated nucleotide changes such as `C180T, A200G`, while Amino Acid Mutations takes gene-qualified residues such as `GP:440G`. INSDC Source is a three-way picker, `Any`, `INSDC Only`, or `Non-INSDC Only`, that scopes results by whether the record has been mirrored to INSDC. The filters combine with AND logic across organism, provenance, and sequence attributes.

To download, click search, select the records you want, and download them. The search returns OPEN records only, so restricted sequences never appear in the results. When a record carries an INSDC accession, Lungfish tries the GenBank path first and appends Pathoplexus metadata; if that retrieval fails, or no INSDC accession exists, it builds a FASTA-backed bundle from the Pathoplexus record instead.

The offered organisms are a fixed catalogue of ten outbreak-relevant pathogens: Mpox virus, Marburg virus, measles, the Sudan and Zaire ebolaviruses, RSV-A and RSV-B, human metapneumovirus, West Nile virus, and Crimean-Congo hemorrhagic fever. A few carry segmented genomes, meaning the organism's sequence is split across several molecules rather than one; Crimean-Congo hemorrhagic fever, for one, splits into `S`, `M`, and `L` segments. Whichever path builds the bundle, the result is the same: a `.lungfishref` reference bundle in the project's `Reference Sequences/` folder with a provenance sidecar. A Pathoplexus import then behaves exactly like an NCBI download in every later chapter.
