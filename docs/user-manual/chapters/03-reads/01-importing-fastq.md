...
**Pairing.** Tells LGE whether each sample is one file, two mate files, or one file whose mates alternate, offering Single-end, Paired-end, and [Interleaved](../../GLOSSARY.md#interleaved-fastq). It defaults to Paired-end when a mate was matched and to Single-end otherwise, and choosing Oxford Nanopore hides it. Single-end imports every file as its own sample even when a mate was matched, naming each bundle after its file, and Interleaved imports each file whole as a paired bundle. On the command line this is `--pairing`, which takes `auto`, `single`, `paired`, or `interleaved`, and its default `auto` matches mates by file name as the sheet does.
