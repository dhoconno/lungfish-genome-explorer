...
<!-- FIXED-IN-2026.9.41: 9 -->
**Pairing.** Shows whether each sample is one file or two mate files, offering Single-end, Paired-end, and [Interleaved](../../GLOSSARY.md#interleaved-fastq). It defaults to Paired-end when a mate was matched and to Single-end otherwise, and choosing Oxford Nanopore hides it. The choice currently has no effect, because LGE pairs by filename alone, so two matched files import as a pair and one file imports as single-end, including a file whose mates alternate. This is a known defect, listed with its workaround in [Known defects in this release](../appendices/troubleshooting.md#known-defects-in-this-release). This setting has no command-line flag.
