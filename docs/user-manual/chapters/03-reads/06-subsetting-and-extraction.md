...
Import the pair following [Importing Sequencing Reads](01-importing-fastq.md), so it becomes the bundle `HG002.chr20.10.0-10.5Mb`, which holds 91,148 reads. Every operation here starts from a bundle in the sidebar rather than a file on disk. R1 is the file holding the first read of each pair, and R2 holds the second. The worked counts in this chapter come from the R1 file alone, 45,574 reads, run from the command line. The window run on the pair searches both files, so expect roughly twice as many reads from the three extraction operations. Subsample by Count still returns exactly the number you ask for.
...
On a paired bundle, the operations in this chapter can split the two mates of a pair in this release, keeping one and dropping the other. This is a known defect, listed with its workaround in [Known defects in this release](../appendices/troubleshooting.md#known-defects-in-this-release).
...
The word roughly matters. The operation decides read by read whether to keep each one, so the count that comes back is close to the fraction rather than exactly on it. A tenth of 45,574 is about 4,557, and three runs at 0.1 on the fixture returned 4,509, 4,516, and 4,562 reads. Each run draws a fresh random sample, which is why the counts differ.
...
Count is exact. Asking for 10,000 reads from the fixture returned exactly 10,000. If the bundle holds fewer reads than you asked for, you get all of them rather than an error.
...
A [regular expression](../../GLOSSARY.md#regular-expression) is a small pattern language, but plain text with this switch on already means "contains this anywhere". Searching the fixture for `HISEQ1` with it on returns all 10,622 reads from that instrument. The characters `. * + ? [ ] ( ) { } ^ $ | \` become pattern instructions once the switch is on, so a period, for instance, comes to mean any single character. Colons and digits are unaffected, so an Illumina identifier can be pasted in as it stands.
...
Square brackets are the piece of pattern syntax worth learning here. Writing `GCCTCCCAAAGTGCTGGGATTACAG[GA]` means "G or A in the final position". That pattern returned 565 reads from the fixture against the literal pattern's 521, picking up 44 reads whose Alu copy differs at that base. IUPAC ambiguity codes such as `R` for A or G are read as the letter `R` itself, so write the alternatives out in brackets instead, as `[AG]`.
...
The gap between 16 and 8 matters more than it sounds. Searching the fixture for the Illumina TruSeq adapter `AGATCGGAAGAGCACACGTC` at the 3' end matched 218 reads at an overlap of 16, 12,058 reads at 12, and 40,507 reads at 8. There are not forty thousand adapter-contaminated reads in this fixture. An 8-base match turns up by chance in almost any 250-base read. Treat a result that keeps most of your reads as a sign the overlap is too low, not as a sign of contamination.
...
The two halves add up, which makes a useful check. On the fixture, keeping matched reads returned 218, turning the switch off returned 45,356, and the two sum to 45,574.

**Search Reverse Complement.** Asks for the reverse complement of your sequence to be searched as well. It is off by default. bbduk already searches both strands, so on the fixture the adapter search returned the same 218 reads with this switch on or off, and you can leave it at the default. In the current release the switch changes nothing. On the command line this is `--search-rc`.
...
Here is what each operation returned on the R1 file's 45,574 reads, run from the command line.

| Operation and setting | Reads out | Share of the input |
|---|---|---|
| Subsample by Proportion, 0.1 | 4,509 to 4,562 across three runs | about 10% |
| Subsample by Count, 10000 | 10,000 | 21.9% |
| Extract Reads by ID, `HISEQ1` with the regex switch on | 10,622 | 23.3% |
| Extract Reads by Motif, the 26-base Alu motif | 521 | 1.1% |
| Select Reads by Sequence, TruSeq adapter at the 3' end | 218 | 0.5% |

The first two rows are requests, and they say only that the sampler did what it was told. The last three are findings about the data. The third says 10,622 reads came off the `HISEQ1` instrument and the remaining 34,952 came off `D00360`. The fourth says about one read in ninety carries this stretch of an Alu element, which fits a human shotgun library, since Alu elements make up about a tenth of the genome and only 26 bases of each copy are being matched. The fifth says half of one percent of reads ran into the adapter, a healthy figure for 250-base reads. A figure in the tens of percent would mean much of the library was shorter than the read length, and the thing to check then is the fragment size the library was built to.
...
Check that a subsample kept the shape of the original. The fixture's reads are 76.7 percent `D00360` and 23.3 percent `HISEQ1`, and one 0.1 subsample came back 76.3 percent and 23.7 percent. The quality and length cards should track the parent's just as closely. A subset whose cards differ noticeably from the parent's was not drawn the way you think.
