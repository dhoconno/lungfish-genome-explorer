...
`lungfish-cli esviritu detect -i <fastq>... -s <sample> [--db <path>] [--paired] [--recursive] [--no-qc] [--extra-args <args>] [-o <dir>]` runs EsViritu for viral identification. `-s` or `--sample` is required and names the output prefixes, and `--db` points at the database directory, which LGE auto-detects when you leave it out. `lungfish-cli esviritu download-db [--force]` installs the viral reference database and `lungfish-cli esviritu db-status` reports whether it is there.
