...
The conventions: this appendix uses `bash` code blocks for canonical commands, `json` blocks for sidecar excerpts, and tables for flag references. Numbers and flag values match the current Lungfish build (0.5.0-alpha4); future versions may adjust defaults, and the truth is whatever the provenance sidecar records for a specific run.
