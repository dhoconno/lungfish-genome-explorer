...
|---|---|---|---|
| Clicking a primer scheme in the sidebar raises "Failed to Open File" with "Unsupported file format" instead of showing the scheme in the Inspector. | [Primer Scheme Bundles](primer-schemes.md), [Primer Trimming an Alignment](../04-alignments/03-primer-trimming.md) | Choose the scheme from the **Primer Scheme** menu of the dialog that uses it. To read its details, right-click the bundle in Finder, choose **Show Package Contents**, and open `manifest.json` in a text editor. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 1 --> |
| After you open a second project into the same window, the Inspector can keep showing the item selected in the first project. | [The Lungfish Genome Explorer Project](../01-foundations/06-the-lungfish-project.md#the-inspector) | Click an item in the new project's sidebar, which refreshes the Inspector. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 2 --> |
| A file added to a folder that was empty may not appear in the sidebar. | [The Lungfish Genome Explorer Project](../01-foundations/06-the-lungfish-project.md#a-tour-of-the-sidebar) | Close the project and open it again. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 4 --> |
...
| A workflow package linked and enabled in the Workflow Library gets no **Tools** menu item, and no menu item opens the Workflow Operations window on its own. | [Running External Workflows](../08-workflows/03-running-external-workflows.md#procedure) | Enable **12S Amplicon Matching** in the Workflow Library, choose **Tools > Genotyping > 12S Amplicon Matching...**, then click the package in the window's list of workflows. | 2026.9.40 |
...
| **Re-root Here** and `lungfish-cli tree reroot` write a new tree bundle with copies of some tips, so rooting the five-tip primate mitochondrial tree on gorilla gives thirteen tips. | [Building Trees](../02-sequences/05-building-trees.md) | Root the tree outside LGE, or treat the re-rooted bundle as a display only. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 8 --> |
...
| In the Variants tab's Search Builder, a Region value with no sequence name, such as `1000-2000`, is ignored without a warning, so the table is not restricted at all. | [Reading the Variants Table](../05-variants/02-reading-the-variant-browser.md) | Write the region with its sequence name, as `chr20:1000-2000`. | 2026.9.40 |
...
| The Kraken 2 taxonomy table's % column tooltip says the value is a percent of classified reads, but it is a percent of all reads, unclassified included. | [Running Kraken 2](../06-classification/02-running-kraken2.md) | Read % as a share of all reads, and take the classified share from the Classified card. | 2026.9.40 |
...
| The **Pairing** popup on the Import FASTQ sheet has no effect, whether the sheet opened from the Import Center or after **Download Selected** in the SRA search. LGE pairs reads by file name, so two matched mate files import as a pair and one file imports as single-end. A single interleaved file therefore imports as single-end, and choosing Single-end for a matched pair still imports a pair. For SRA downloads the **Compression Tool** popup is ignored too. | [Importing Sequencing Reads](../03-reads/01-importing-fastq.md), [Downloading Reads from the SRA](../03-reads/02-downloading-from-sra.md) | To import mates as a pair, supply them as two files whose names match, such as `_R1` and `_R2`. To import a pair as single-end, import one file at a time. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 9 --> |
| On a paired bundle whose mates are stored together in one file, most operations under **Tools** split or reorder the mates. Subsampling, extracting by motif, selecting reads by sequence, removing duplicates, contaminants, low-complexity reads, human reads, and ribosomal RNA are affected. Extracting by read ID and merging overlapping pairs keep the pairs. | [Decontamination](../03-reads/05-decontamination.md), [Subsetting and Extraction](../03-reads/06-subsetting-and-extraction.md) | Treat the output as single-end reads, or import the two mate files separately and run the operation on each. | 2026.9.40 <!-- FIXED-IN-2026.9.41: 10 --> |
...
| **Call Variants** always runs bcftools as a haploid caller, one copy per genome, and adds `--ploidy 1` after your Extra arguments, so a `--ploidy 2` there has no effect. On a diploid human sample most heterozygous sites are missed and every genotype reads `1`. | [Calling Variants](../05-variants/01-calling-variants-from-amplicons.md), [Reading the Variants Table](../05-variants/02-reading-the-variant-browser.md) | For diploid genotypes use [HaplotypeCaller](../06-human-germline-variants/01-haplotype-caller.md), or run bcftools outside LGE. | 2026.9.40 |
