...
Two things stay hidden or pale grey until something is turned on, and neither says so loudly.
...
| A command with its own `--threads` option ignores the value you give, because the program-wide `--threads` option takes it first, so the command runs with its default thread count. `variants phase --threads 4` plans GATK with one thread, and `fastq entropy-filter --threads 2` runs with four. `fastq demultiplex`, `fastq pbaa-cluster`, `fastq savont-cluster`, `fastq ont-fluidigm-samples`, and `fastq ont-pacbio-barcode-demux` declare the option the same way. | [HaplotypeCaller](../06-human-germline-variants/01-haplotype-caller.md), [CLI Reference](cli-reference.md) | None from the command line. The result is otherwise correct. | 2026.9.40 |
