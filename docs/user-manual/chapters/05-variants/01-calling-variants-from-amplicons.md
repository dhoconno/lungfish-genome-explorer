...
Click **bcftools** in the tool sidebar. The section titled "bcftools Settings" opens with the line "bcftools will run mpileup and call as an orthogonal cross-check on the selected BAM." Orthogonal there means independent, a second opinion arrived at by a different route. Under it sits one control, **Ploidy**, offering Haploid and Diploid, with a caption saying which one LGE picked for this bundle and why. On the fixture the caption reads that Diploid came from the assembly name, because the bundle was imported from `GRCh38.chr20.10.0-10.5Mb.fasta`, and Diploid is right for a human sample. Check that the Alignment Track menu names your minimap2 track. The Output Variant Track Name field has filled itself in with the track name, a bullet, and the caller name, giving "minimap2 Mapping • bcftools". Leave every field alone and click **Run**.
...
**Ploidy.** Shown for bcftools alone, because bcftools is the one caller here that writes genotypes. [Ploidy](../../GLOSSARY.md#ploidy) is how many copies of each chromosome the sample carries, and it decides whether a heterozygous genotype such as `0/1` can be written at all. Haploid passes `--ploidy 1` to `bcftools call` and suits a virus or a bacterium. Diploid passes `--ploidy 2` and suits a human, a macaque, or any other eukaryote. LGE picks the default from the bundle's own metadata, reading an NCBI Virus record, a GenBank division, the organism name, or a well-known assembly name such as GRCh38, and falls back to Haploid when nothing identifies the organism, so a FASTA slice imported under an uninformative name comes up Haploid and the caption says so. Set it to Diploid yourself in that case. A run that returns almost no `0/1` genotypes on a human sample usually means it was called haploid. On the command line this is `--ploidy`, taking `1` or `2`, and leaving the flag off derives the value the same way the dialog does.

**Extra arguments.** Inserts your own text straight into the caller's command line, right after the subcommand and ahead of the arguments LGE builds, so it reaches `bcftools call`, `lofreq call`, or `ivar variants` as typed. It defaults to empty, and it is the only route to any bcftools or LoFreq setting the dialog does not expose, with one exception. A `--ploidy` typed here for bcftools is refused, and the Readiness line tells you to use the Ploidy control instead. Use it to give LoFreq a real depth floor with `--min-cov 50`, or to switch LoFreq's indel calling on with `--call-indels`, which makes LGE run an extra `lofreq indelqual` pass over a copy of the BAM first, a step that writes per-base quality scores for insertions and deletions into the copy so LoFreq has something to judge them by, and which adds a second pass over the whole alignment to the run. On the command line this is `--extra-args`.
