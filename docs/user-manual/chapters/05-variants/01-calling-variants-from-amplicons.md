...
- **bcftools** asks which base best explains the pileup at each position. LGE runs it as a haploid caller, meaning it models one copy of each chromosome, which suits a virus or a mitochondrion. On a diploid sample such as a human, which carries two copies, it misses most [heterozygous](../../GLOSSARY.md#heterozygous) sites, where only one copy carries the change, and writes every genotype as `1`, meaning the one copy it modelled carries the alternate base. For diploid human genotypes, use [HaplotypeCaller](../06-human-germline-variants/01-haplotype-caller.md).
...
This run shows what a haploid caller does to a diploid sample, which the comparison with LoFreq in step 4 makes visible. Click **bcftools** in the tool sidebar. Its section holds no controls, only the line "bcftools will run mpileup and call as an orthogonal cross-check on the selected BAM", where orthogonal means a second opinion reached by a different route. Check that the Alignment Track menu names your minimap2 track. The Output Variant Track Name fills in as "minimap2 Mapping • bcftools". Leave every field alone and click **Run**.
...
**Extra arguments.** Passes text straight to the caller without LGE checking it, placed right after `bcftools call`, `lofreq call`, or `ivar variants`. The default is empty, which is right for almost every run. Use it for an option the dialog does not show, such as `--call-indels` to switch on LoFreq's indel calling, which adds a preparation pass over a copy of the BAM. It cannot change bcftools' ploidy, because LGE always adds `--ploidy 1` after your text. On the command line this is `--extra-args`.

## Reading the results

Run with every setting left alone, bcftools writes 514 rows and LoFreq writes 862, at 861 positions, because LoFreq writes two rows at one position where it saw two different alternate bases. Neither caller samples reads at random, so the same alignment, reference, and settings give the same rows every time.

Of the 514 bcftools rows, 373 are single-base substitutions and 141 are [indels](../../GLOSSARY.md#indel), insertions or deletions. All 862 LoFreq rows are substitutions, because LoFreq calls no indels unless asked. LoFreq reports more rows despite that, because the haploid bcftools run leaves out most sites where only one of this person's two copies carries the change.

The [FILTER](../../GLOSSARY.md#filter) column reads `PASS` when a row cleared the caller's filters and a bare `.` when no filter was applied, as [FILTER, and the flags callers actually write](../01-foundations/05-variants-and-vcf.md#filter-and-the-flags-callers-actually-write) explains. Every bcftools row reads `.` and every LoFreq row reads `PASS`, which is why the **PASS** chip, a one-click filter button above the table, hides the whole bcftools track, as [Step 4 of Reading the Variants Table](02-reading-the-variant-browser.md#step-4-filter-with-the-preset-chips) shows. The two files also differ in shape. The bcftools VCF carries one sample column, named HG002, holding a [genotype](../../GLOSSARY.md#genotype) of `1` on every row. The LoFreq VCF has no sample column and reports [allele frequency](../../GLOSSARY.md#allele-frequency) in its `INFO` field instead.

Each variant track lives inside the reference bundle's `variants/` folder as a `.vcf.gz` file, its `.vcf.gz.tbi` index, and a `.db` database the table queries, beside a provenance record with the same stem. For the bcftools track these come to about 22 KB, 365 bytes, and 1.6 MB. The shared name is the track id, `vc-` followed by a long identifier LGE assigns. The window does not show it. To see the folder, Control-click the bundle in Finder and choose Show Package Contents.
...
Third, check the calls against the benchmark. Comparing positions only, 452 of the 514 positions bcftools called and 808 of the 861 positions LoFreq called match a position in the 961-call benchmark. Out of the benchmark's 961 positions, that is 47 percent for bcftools and 84 percent for LoFreq, which finds far more of the benchmark because it keeps heterozygous sites. Most of each caller's own calls match, 88 and 94 percent, which is what a sound call set looks like. A position match is not genotype agreement, and a real accuracy assessment needs a separate benchmarking program such as `hap.py`, which LGE does not ship and this manual does not cover.
