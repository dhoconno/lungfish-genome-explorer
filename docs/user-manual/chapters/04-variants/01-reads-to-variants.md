...
1. Create a new Lungfish project to hold the workflow's outputs. From the Welcome window choose `Create Project`, or from the menu bar choose `File > New Project`. Name the project (`SARS-CoV-2 SRR36291587` is a sensible label) and save it under your `Documents` folder. Lungfish opens a new window titled with the project name. Then download the reference. Choose `Tools > Search Online Databases > Search NCBI…` to open the database search dialog. In `GenBank & Genomes`, leave `Mode` set to `Nucleotide`, type `MN908947.3`, and click `Search`. Select the `MN908947.3` result, leave `Include GFF3 Annotations` checked, and click `Download Selected`. Lungfish downloads the sequence and annotation, builds an annotated reference bundle, and imports it into the project. The reference appears in the left sidebar under `Reference Sequences > MN908947.3`. Behind the dialog the CLI equivalent is `lungfish fetch ncbi MN908947.3 --fetch-format fasta`, `lungfish fetch ncbi MN908947.3 --fetch-format gff3`, then `lungfish bundle create --fasta MN908947.3.fasta --annotation MN908947.3.gff3 --compress`.
...
The codon teaching moment lands at position 28881. The N-protein open reading frame puts positions 28881 and 28882 in the codon that encodes amino acid 203 of the nucleocapsid, while position 28883 starts the next codon. The iVar TSV-to-VCF converter, when handed the downloaded GFF3, groups the same-codon pair into one haplotype row with REF `GG` and ALT `AA`; the following `28883 G>C` change remains its own row because it belongs to the next codon. LoFreq still emits three one-base rows. This is the moment you realize that "one variant per row" is a presentation choice with biological consequences. Two callers can describe the same change. Whether they describe it as one row or several depends on whether they consult an annotation file before they write VCF.
