...
This chapter teaches three concrete things. First, where sidecars live and how to read one. Second, how to export a project's full provenance trail as a runnable Shell script, a Nextflow pipeline, a Snakemake workflow, or a methods-section paragraph for a paper. Third, what a sidecar does and does not capture. It captures the command and the inputs; it does not pin the conda environment hash by default, so cross-machine bit-identical [reproducibility](../../GLOSSARY.md#reproducibility) also requires running with the same plugin pack version.
...
| [Methods Section](../../GLOSSARY.md#methods-export) | A plain-prose paragraph in Markdown, with tool names and versions inline | Methods sections in papers and clinical reports |
