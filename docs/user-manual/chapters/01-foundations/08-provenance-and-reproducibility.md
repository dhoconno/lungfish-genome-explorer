...
task: Read a Lungfish provenance sidecar and export a workflow as a shell script, Nextflow, or Snakemake.
...
Every Lungfish operation that produces a file (a download, a mapping, a primer trim, a variant call) writes a provenance sidecar alongside the output. The sidecar is a JSON file that records the resolved tool versions, the exact command that ran, the input files with checksums and sizes, the runtime identity (machine, OS, CPU), the per-step exit status and stderr, and the wall time. Two analysts running the same workflow on the same inputs can compare sidecars to verify that they got the same answers for the same reasons.
...
A sidecar is a single JSON document with seven top-level keys. The shape is stable across operations; an alignment sidecar and a variant-call sidecar have the same structure but different `command` and `tool` fields. The example below is trimmed for readability; production sidecars include a few additional fields (full file paths, environment variables, a `schema_version`) that are omitted here.
...
```json
{
  ...
  "tool": {
    ...
    "conda_env": "/Users/me/.lungfish/conda/envs/ivar"
  },
  "steps": [
    { "command": "samtools mpileup -aa -A -d 600000 -B -Q 20 -q 0 -f MN908947.3.fasta SRR36291587.trimmed.bam", "exit_status": 0 },
    { "command": "ivar variants -p variants -q 20 -t 0.05 -m 10 -r MN908947.3.fasta -g MN908947.3.gff3", "exit_status": 0 }
  ]
}
```
...
| Nextflow | `main.nf` with one process per Lungfish step plus `nextflow.config` and a `containers/` manifest | Running on a cluster; scaling out across many samples |
...
Take the variant-calling step from the [pilot chapter](../05-variants/01-calling-variants-from-amplicons.md). When that step finishes, Lungfish writes `variants/SRR36291587.ivar.vcf.gz.lungfish-provenance.json` next to the VCF inside the reference bundle. The sidecar for this step is structurally identical to the example above; the relevant fields are the resolved command, the trimmed BAM checksum, and the iVar tool version.
...
# Activate the Lungfish-managed iVar environment.
...
# Convert iVar TSV to VCF.gz with Lungfish's helper script.
...
- Lungfish `version` is the same in both sidecars (top-level `version` field).
- Plugin pack name and version match (`tool.plugin_pack` and `tool.plugin_pack_version`).
- Every input checksum in `inputs[]` matches between runs (SHA-256 over file contents).
- The `runtime.arch` field matches (arm64 vs x86_64; some tools are not bit-identical across architectures).
- The `runtime.cpu_threads` count matches if the tool's output depends on thread count (SPAdes assembly is the common offender; iVar and bcftools are not).
...
- The conda environment hash. Lungfish records the plugin pack name and version, which pins the environment recipe, but does not hash every package in the resolved environment. Two machines on the same plugin pack version will usually have identical environments, but conda's solver can pick different transitive dependencies if the upstream channel state has changed.
...
Lungfish writes the sidecar regardless of which audience asked for it, which means the audit trail is always there when someone asks. The export menu is the surface that turns the trail into something portable.
