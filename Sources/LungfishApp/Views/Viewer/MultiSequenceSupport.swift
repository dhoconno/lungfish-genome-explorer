...
public extension Notification.Name {
    ...
    // `metagenomicsMetadataImportRequested` now lives in the LungfishKit kernel
    // (shared by the metagenomics + genotype document sections and the Inspector).

    // The genotype-prefixed `Notification.Name` constants now live in the
    // LungfishGenotypeUI leaf (`GenotypeNotifications.swift`). The string values
    // are unchanged, so dispatch is identical across the module boundary.
