...
enum ONTDirectoryImportRecipe: CaseIterable {
    ...
    var displayName: String {
        ...
        case .flattened:
            return "Flatten ONT barcode directories"
        }
    }
...
@MainActor
final class ONTImportOptionsAccessoryController: NSObject {
    let recipePopup: NSPopUpButton
    let recipeExplanationLabel: NSTextField
    let barcodeDefinitionPopup: NSPopUpButton
    let chooseBarcodeDefinitionButton: NSButton
    let barcodeDefinitionStatusLabel: NSTextField
    ...
    init(
        recipePopup: NSPopUpButton,
        recipeExplanationLabel: NSTextField,
        barcodeDefinitionPopup: NSPopUpButton,
        chooseBarcodeDefinitionButton: NSButton,
        barcodeDefinitionStatusLabel: NSTextField,
        optimizeStorageButton: NSButton,
        includeUnclassifiedButton: NSButton?,
        importButton: NSButton,
        projectURL: URL?,
        barcodeDefinitionCandidates: [URL]
    ) {
        self.recipePopup = recipePopup
        self.recipeExplanationLabel = recipeExplanationLabel
        self.barcodeDefinitionPopup = barcodeDefinitionPopup
        self.chooseBarcodeDefinitionButton = chooseBarcodeDefinitionButton
        self.barcodeDefinitionStatusLabel = barcodeDefinitionStatusLabel
        self.optimizeStorageButton = optimizeStorageButton
        self.includeUnclassifiedButton = includeUnclassifiedButton
    ...
    var selectedRecipe: ONTDirectoryImportRecipe {
        let title = recipePopup.titleOfSelectedItem ?? ONTDirectoryImportRecipe.sampleSplit.displayName
        return ONTDirectoryImportRecipe.allCases.first { $0.displayName == title } ?? .sampleSplit
    }
    ...
    @objc func recipeChanged(_ sender: NSPopUpButton) {
        updateControls()
    }

    @objc func barcodeDefinitionChanged(_ sender: NSPopUpButton) {
        updateControls()
    }
    ...
    private func selectBarcodeDefinition(_ url: URL) {
        if !barcodeDefinitionCandidates.contains(where: { $0.standardizedFileURL == url.standardizedFileURL }) {
            ...
            barcodeDefinitionCandidates.sort {
                Self.displayPath(for: $0, relativeTo: projectURL)
                    .localizedStandardCompare(Self.displayPath(for: $1, relativeTo: projectURL)) == .orderedAscending
            }
            populateBarcodeDefinitions()
        }
    ...
    private func populateBarcodeDefinitions() {
        ...
        if barcodeDefinitionCandidates.isEmpty {
            barcodeDefinitionPopup.addItem(withTitle: "Choose a barcode definition...")
            barcodeDefinitionPopup.lastItem?.isEnabled = false
        } else {
            for url in barcodeDefinitionCandidates {
                barcodeDefinitionPopup.addItem(withTitle: Self.displayPath(for: url, relativeTo: projectURL))
    ...
    private func updateControls() {
        ...
        if recipe == .sampleSplit {
            if let selectedBarcodeDefinitionURL {
                ...
                barcodeDefinitionStatusLabel.stringValue = "Choose a barcode definition before importing."
