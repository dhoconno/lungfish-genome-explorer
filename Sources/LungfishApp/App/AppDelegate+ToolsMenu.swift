...
extension AppDelegate {
    ...
    /// mid-flight).
    ///
    /// The emptiness check is recursive by ONE level: `precomputedMapping-
    /// BatchOutputDirectories` pre-creates every child's sample directory
    /// (empty) before any child runs, so a shallow top-level listing would
    /// always see N directory entries -- even when every child failed
    /// before writing anything, or a bundle was never reached because the
    /// batch was cancelled mid-flight. A top-level entry counts as "content"
    /// (and blocks removal) iff it is EITHER a non-directory (any flat file
    /// some future producer might leave at the batch root, `analysis-
    /// metadata.json` excluded) OR a directory that itself has at least one
    /// entry. A pre-created-but-still-empty child directory does not block
    /// removal; a child directory containing partial output (from a child
    /// that failed AFTER writing something) does. This deliberately does
    /// NOT remove individual empty child directories on its own (that would
    /// be child-side cleanup, which the design rules out -- a failed
    /// child's partial output must still block the whole batch from being
    /// removed) -- it only decides whether the WHOLE batch directory,
    /// including its still-empty children, is removable as a unit.
    ///
    ...
    static func cleanupBatchDirectoryIfEmpty(_ batchDirectory: URL) {
        ...
        let hasContent = entries.contains { entry in
            guard entry.lastPathComponent != AnalysesFolder.metadataFilename else { return false }
