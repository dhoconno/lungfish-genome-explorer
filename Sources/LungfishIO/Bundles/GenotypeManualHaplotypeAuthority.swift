    ...
    public static func evaluate(
        ...
        case let (.some(kind), .some(mode)):
            guard legacyKind == kind else {
                return .ineligible(
                    reason:
                        "The typed workflow kind and result kind disagree."
                )
            }
        ...
        case (nil, nil):
            guard let legacyKind else {
                return .ineligible(
                    reason:
                        "This legacy result schema is not a recognized genotype-only MHC workflow."
                )
            }
        ...
        guard !manifestDeclaresHaplotyping else {
            return .ineligible(
                reason:
                    "The genotype-only workflow declaration and authoritative haplotyping fields disagree."
            )
        }
