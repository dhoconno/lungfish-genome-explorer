...
public struct ProvenanceExporter: Sendable {
    ...
    public func exportShell(_ run: WorkflowRun) -> String {
        ...
        if !inputs.isEmpty {
            ...
            for (i, input) in inputs.enumerated() {
                ...
                s += "\(varName)=\(shellEscape(input.path))"
    ...
    public func exportPython(_ run: WorkflowRun) -> String {
        ...
        for (i, step) in run.steps.enumerated() {
            ...
            let args = replayArguments(step) ?? []
            let pyArgs = args.map(pythonDoubleQuoted)
            s += "    run_step(\(pythonDoubleQuoted(step.toolName)), [\n"
            for arg in pyArgs {
                s += "        \(arg),\n"
            }
            s += "    ])\n\n\n"
    ...
    public func exportMethods(_ run: WorkflowRun) -> String {
        ...
        for step in run.steps where step.isSuccess {
            ...
            if isRetainedSelectionReplay(step) {
                toolSentences.append("The selected export bytes were retained with checksummed selection metadata. Reproduction uses retained-selection snapshot byte replay and does not rerun upstream analysis.")
                continue
            }
        ...
        s += "Reproducibility\n"
        s += "---------------\n\n"
        if run.steps.contains(where: isRetainedSelectionReplay) {
