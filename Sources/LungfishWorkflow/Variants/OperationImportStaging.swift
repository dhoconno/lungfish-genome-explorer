...
public struct OperationImportStaging: Sendable {
    public let directory: URL

    public init(parentDirectory: URL, operationID: UUID) throws {
    ...
    public func finishCommittedImport(cleanupAction: (() throws -> Void)? = nil) -> String? {
        do {
    ...
    public struct RecoveryRequired: Error, LocalizedError {
        public let directory: URL
        public let originalErrorDescription: String
        public let restorationErrorDescription: String
        public private(set) var additionalRecoveryURLs: [URL]

        public init(directory: URL, originalError: Error, restorationError: Error, additionalRecoveryURLs: [URL] = []) {
        ...
        public func retainingRecoveryURLs(_ urls: [URL]) -> RecoveryRequired {
            var result = self
    ...
    public struct SQLitePublication {
        public let staging: OperationImportStaging
        ...
        public var stagedURL: URL { staging.directory.appendingPathComponent(filename) }
        private var previousSnapshotURL: URL { staging.directory.appendingPathComponent("previous-" + filename) }

        public func publish(beforeRollback: () throws -> Void = {}, commit: () throws -> Void) throws {
    ...
    public func prepareSQLiteCopy(filename: String, from destination: URL) throws -> SQLitePublication {
